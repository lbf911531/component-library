...
var MAX_SAFE_INTEGER = Number.MAX_VALUE || Math.pow(2, 308) - 1;
...
var InputNumber = (function (_React$Component) {
  ...
  InputNumber.prototype.componentWillReceiveProps =
  ...
  // add -custom
  // inputting 为true，允许出现以 小数点. 结尾，如 12.  可以value是负号 -
  InputNumber.prototype.getNumber = function (params, inputting) {
    const { maxLength, precision, min } = this.props;
    const signReg = min >= 0 ? '' : '-?'; // min >=0 时 不能输入负数
    let numberReg = '';
    if (precision <= 0) {
      numberReg = inputting ? '[0-9]*' : '[0-9]+';
    } else if (inputting) {
      // value可以是 - 或者 12.
      numberReg = `([0-9]+[\.]?[0-9]{0,${precision || ''}})?`;
    } else {
      numberReg = `[0-9]+([\.]?[0-9]{1,${precision || ''}})?`;
    }
    const number = String(params).match(new RegExp(`${signReg}${numberReg}`));
    if (!number) return '';
    if (maxLength) {
      const list = (number[0] || '').split('.');
      list[0] = list[0].slice(0, maxLength);
      return list.join('.');
    }
    return number[0] || '';
  };

  InputNumber.prototype.getCurrentValidValue = function getCurrentValidValue(
    value,
  ) {
    var val = value;
    if (val === '') {
      val = '';
    } else if (!this.isNotCompleteNumber(this.getNumber(val))) {
      // 注释-custom  else if (!this.isNotCompleteNumber(parseFloat(val, 10)))
  ...
  InputNumber.prototype.getValidValue = function getValidValue(value) {
    ...
    // 注释-custom var val = parseFloat(value, 10);
    var val = this.getNumber(value);
  ...
  InputNumber.prototype.setValue = function setValue(v, callback) {
    ...
    // 注释-custom var newValue = this.isNotCompleteNumber(parseFloat(v, 10)) ? null : parseFloat(v, 10);
    var newValue = this.isNotCompleteNumber(this.getNumber(v))
      ? null
      : this.getNumber(v);
    ...
    var newValueInString = newValue
      ? new BigNumber(newValue).toFixed(precision)
      : '' + newValue;
  ...
  InputNumber.prototype.getMaxPrecision = function getMaxPrecision(
    currentValue,
  ) {
    var ratio =
  ...
  InputNumber.prototype.isNotCompleteNumber = function isNotCompleteNumber(
    num,
  ) {
    return (
      isNaN(num) ||
  ...
  InputNumber.prototype.toNumber = function toNumber(num) {
    ...
    if (isValidProps(this.props.precision)) {
      return num && new BigNumber(num).toFixed(this.props.precision);
    }
    return num;
  };
  ...
  InputNumber.prototype.upStep = function upStep(val, rat) {
    ...
    const bigPrecisionFactor = new BigNumber(precisionFactor);
    const value1 = bigPrecisionFactor.times(val);
    const value2 = bigPrecisionFactor.times(step).times(rat);
    const result = value1.plus(value2).div(precisionFactor).toFixed(precision);
  ...
  InputNumber.prototype.downStep = function downStep(val, rat) {
    ...
    const value1 = bigPrecisionFactor.times(val);
    const value2 = bigPrecisionFactor.times(step).times(rat);
    const result = value1.minus(value2).div(precisionFactor).toFixed(precision);
    return this.toNumber(result);
  };
  ...
  InputNumber.prototype.render = function render() {
    ...
    var prefixCls = props.prefixCls,
      ...
      rest = (0, _objectWithoutProperties3['default'])(props, [
        'prefixCls',
        'disabled',
        'readOnly',
        'useTouch',
    ...
    var classes = (0, _classnames2['default'])(
      ((_classNames = {}),
      (_classNames[prefixCls] = true),
      (_classNames[props.className] = !!props.className),
      (_classNames[prefixCls + '-disabled'] = disabled),
      (_classNames[prefixCls + '-focused'] = this.state.focused),
      _classNames),
    );
    var upDisabledClass = '';
...
InputNumber.propTypes = {
  ...
  step: _propTypes2['default'].oneOfType([
    _propTypes2['default'].number,
    _propTypes2['default'].string,
  ]),
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.onChange = function (e) {
    ...
    // 注释-custom  _this3.rawInput = _this3.props.parser(_this3.getValueFromEvent(e))
    _this3.rawInput = _this3.getNumber(
      _this3.props.parser(_this3.getValueFromEvent(e)),
      true,
    );
